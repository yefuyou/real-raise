...
function makePrompt(request: StartAnalysisRequest, sources: SourceReference[]): string {
  ...
  return [
    ...
    'B. 区分用户输入、本地确定性计算、官方观察值和派生估算四类信息。',
    'C. 给出最多 3 个不改变本地数字的情景解释。',
...
type CacheEntry = { material: string; insight: string; artifacts: Array<[string, string]> }
...
function ensureLocalArtifacts(task: VendorTask): void {
  if (!task.artifacts.has('evidence.csv')) {
    task.artifacts.set('evidence.csv', buildEvidenceCsv(task.request, task.sources))
  }
  if (!task.artifacts.has('analysis-manifest.json')) {
    task.artifacts.set(
  ...
  if (!task.started) {
    ...
    if (task.cachedResult) {
      ...
      setTimeout(() => {
        ...
        emit(task, { type: 'started', taskId: task.id })
        emit(task, { type: 'completed', taskId: task.id, insight: cached.insight, sources: task.sources })
      }, 0)
    } else {
      void runTask(task)
    }
