...
async function main() {
  ...
  await runTest('12.5 运行状态矩阵：本地隐藏 SSO，产品不暴露 Judge/BYOK 入口', () => {
    ...
    assert(insightSource.includes('id="partner-analysis-model-select"'), '登录用户必须保留独立的模型选择器')
    assert(insightSource.includes('DeepSeek V4 Flash（省额度）'), '模型选择器必须提供 Flash')
    assert(insightSource.includes('DeepSeek V4 Pro（高质量）'), '模型选择器必须提供 Pro')
    assert(insightSource.includes("handleStartInsight(undefined, 'replay')"), '登录前后都必须保留显式回放入口')
