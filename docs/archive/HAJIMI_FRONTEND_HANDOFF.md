...
8. 详细模式的六类 CPI 文案必须读取 `DEFAULT_CATEGORY_CPI_RATES`，当前基准是 2026H1；不要在组件里保留 2025 年的硬编码说明。
...
- 城市选择必须放在工资/扣缴输入流程中，默认合肥；桌面端在输入区域右侧或相邻位置显示当前城市基准卡，移动端自然堆叠。
- 当前城市基准与历史宏观对比不得同时纵向展开；在同一个“价格基准”区域内使用两个 Tab：`当前城市`（默认）和 `历史参考`。切换 Tab 时替换原位置内容，不在页面下方追加第二张长卡。
- `当前城市` Tab 展示所选城市的当期价格分类和来源；`历史参考` Tab 承载 2021–2025 全国历史对比及合肥 2024 历史样本。合肥 2024 历史样本从当前 Tab 的内嵌说明中移除，避免重复出现。
- 不向普通用户展示 30 城覆盖矩阵、覆盖地图或 `A-history / B-current / C-fallback` 内部术语；这些只留给数据层和 Codex 验收。
- 城市选择器先接 `src/data/cityBenchmarks.ts` 的解析结果，不要自行抓取网页。
- 显示城市、期间、价格分类和来源链接；缺失数据只用一行自然语言说明，不展示核验数量、回退数量或内部覆盖层级。
- `record.scope === 'city'` 才能标为城市原值；`usedFallback === true` 必须展示“已回退到全国基准”。
- 合肥 2024 是 `A-history`，上海 2026H1 只有综合 CPI，上海 202606 八类是月度样本；期间不能混写。
- 城市目录中的待核验城市只能展示待核验/全国回退状态，不得自行补数字。
- 住房/固定支出仍是用户输入，不被城市 CPI 覆盖。
...
- 用户住房/固定支出优先使用用户输入；官方“居住”类价格只作为背景参照。
