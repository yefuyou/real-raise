# InfiniSynapse 模型选择 API 调研（2026-07-26 历史接口资料）

> [!NOTE]
> 本文中的接口调研属于历史资料；“模型选择已退役”的旧判断已经被取代。
> 当前产品为 Partner 登录用户保留模型选择，回放不受选择影响。
> 现行决定见 [`decisions/0005-partner-model-selection.md`](../decisions/0005-partner-model-selection.md)。
> 禁止据此恢复浏览器 Key、Judge 或开发者面板。
