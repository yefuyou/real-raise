# BYOK + 回放模式历史归档（禁止作为产品入口）

> 2026-07-30：BYOK、自带 Key、Judge 和开发者折叠入口已从目标产品中退役，
> 不得重新接回 UI。当前用户只看到两条报告路径：真实回放、登录后 Partner
> 实时模式；Partner 登录用户仍可选择实时任务模型。本文件仅保留迁移背景和
> Replay v2 兼容记录；Worker 的历史 Judge 接口属于待删除实现漂移。
...
`replay.v2`；完整来源与迁移规则见 [REPLAY_PROVENANCE.md](../architecture/REPLAY_PROVENANCE.md)。
...
页面不提供 API Key 输入、Judge 或 Mock 模式入口；模型选择只属于 Partner 实时任务。
